...
parser.add_argument('--folder_name_extension', nargs='?', default="_3c", type=str, help='Set folder name extension')
...
fles = sc.FederatedLearningEnvironments
last_fle = fles[len(fles)-1]
last_fle_name = last_fle.name

# write last_fle_name to file
with open(os.path.join(os.getcwd(),"results", sc.name,"last_fle_name.txt"), 'w') as f:
    f.write(last_fle_name)
