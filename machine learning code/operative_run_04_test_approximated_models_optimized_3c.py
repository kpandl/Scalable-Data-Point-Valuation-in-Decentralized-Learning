...
parser.add_argument('--dscmode', nargs='?', default=8, type=int, help='Specify the dataset collection mode')
...
parser.add_argument('--measure_elapsed_time', dest='measure_elapsed_time', action='store_true')
parser.set_defaults(measure_elapsed_time=False)
...
last_fle = list(sc.coalitions[-1])
last_fle_length = len(last_fle)
client_ids_from_last_fle = list(range(last_fle_length))

start = time.time()

sc.create_federated_learning_environments(reverse_list=False, start_environment=args.start_environment, end_environment=args.end_environment, weighted_aggregation=args.weighted_aggregation, combine_weighted_and_unweighted_aggregation=args.combine_weighted_and_unweighted_aggregation, function_to_run_directly=function_to_run, client_ids_from_last_fle=client_ids_from_last_fle)
...
if(args.measure_elapsed_time):
    print("Runtime for creating FLEs: ", runtime_tracker)
    path_runtime_file = os.path.join(os.getcwd(),'results',sc.name,"runtime_tracker_OR.csv")
    with open(path_runtime_file, 'a') as f:
        writer = csv.writer(f)
        writer.writerow(["testing_artificial_models",runtime_tracker])