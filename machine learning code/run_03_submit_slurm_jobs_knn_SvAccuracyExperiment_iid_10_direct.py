...
SV_names = ["Shapley_original_method_predictive_performance.csv", "Shapley_federated_performance.csv", "Shapley_LR_method_predictive_performance.csv", "Shapley_performance.csv"]
final_path_list_age = []
...
for SV_name in SV_names:
    for i, _ in enumerate(final_path_list):
        ...
        for seed in seed_list:
            df = pd.read_csv(os.path.join(os.getcwd(), "results", "default"+str(seed)+final_path_list[i], SV_name))
            df_list.append(df)

        Path(os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i])).mkdir(parents=True, exist_ok=True)
        
        path_means_and_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], SV_name[:-4] + "_means_and_hs.csv")
        path_means = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], SV_name[:-4] + "_means.csv")
        path_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], SV_name[:-4] + "_hs.csv")
