...
final_path_list_gender = ["_reduced_maxtranslation_0.1_3c_iid"]
final_path_list_age = []
...
for i, _ in enumerate(final_path_list):
    ...
    column_names = ["method", "type", "cosine_similarity"]

    for seed in seed_list:
        df = pd.read_csv(os.path.join(os.getcwd(), "results", "default"+str(seed)+final_path_list[i], "cosine_similarity.csv"))
        df_list.append(df)

    Path(os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i])).mkdir(parents=True, exist_ok=True)
    
    path_means_and_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_cosine_similarities_means_and_hs.csv")
    path_means = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_cosine_similarities_means.csv")
    path_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_cosine_similarities_hs.csv")
    ...
    with open(path_means_and_hs, 'a', newline="") as logfile, open(path_means, 'a', newline="") as logfile_means, open(path_hs, 'a', newline="") as logfile_half_spans:
        ...
        for row in df_list[0].iterrows():
            ...
            logwriter.writerow([this_row[i] for i in column_names[:2]]+this_row_combined_values)
            logwriter_means.writerow([this_row[i] for i in column_names[:2]]+this_row_means)
            logwriter_half_spans.writerow([this_row[i] for i in column_names[:2]]+this_row_half_spans)