...
for i in range(num_jobs):
    ...
    with open("job_knn_scalability_exp1_3_"+str(seeds_start)+".sh", "w+") as file:
        file.writelines(line_list)

    time.sleep(0.8)
    os.system("sbatch -p single --cpus-per-gpu=16 --time=48:00:00 --mem=236000 --gres=gpu:1 " + "job_knn_scalability_exp1_3_"+str(seeds_start)+".sh")