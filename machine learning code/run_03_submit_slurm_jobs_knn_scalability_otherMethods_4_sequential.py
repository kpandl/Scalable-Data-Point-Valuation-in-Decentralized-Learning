...
for i in range(num_jobs):
    ...
    for j in range(len(client_counts)):
        ...
        line_list.append("python -u operative_run_05_compute_bias_metrics_sex_3c.py --name_of_test_folder=default0_reduced_maxtranslation_0.1_knn_scalability_" + str(client_counts[j+4*i]) + "_clients\n")


    with open("job_knn_scalability_exp1_4_"+str(seeds_start)+".sh", "w+") as file:
        file.writelines(line_list)

    time.sleep(0.8)
    os.system("sbatch -p single --ntasks=1 --time=72:00:00 --mem=236gb " + "job_knn_scalability_exp1_4_"+str(seeds_start)+".sh")