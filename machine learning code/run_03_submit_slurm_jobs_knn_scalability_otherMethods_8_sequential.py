...
for i in range(num_jobs):
    ...
    for j in range(len(client_counts)):
        line_list.append("python3 -u operative_run_09_compute_SV_LR_coalitions_federated_ensembling_3c.py --seed=0 --start_environment=0 --end_environment=" + str(2**client_counts[j+4*i]-1) + " --name_of_test_folder=default0_reduced_maxtranslation_0.1_knn_scalability_" + str(client_counts[j+4*i]) + "_clients_b --dscmode=" + str(dsc_modes_full[j+4*i]) + " --skip_bias_gender\n")

    with open("job_knn_scalability_exp1_8_"+str(seeds_start)+".sh", "w+") as file:
        file.writelines(line_list)

    time.sleep(0.8)
    #os.system("sbatch -p single --ntasks=1 --time=72:00:00 --mem=236gb " + "job_knn_scalability_exp1_8_"+str(seeds_start)+".sh")
    #os.system("sbatch -p single --ntasks=1 --time=72:00:00 --mem=2010gb " + "job_knn_scalability_exp1_8_"+str(seeds_start)+".sh")