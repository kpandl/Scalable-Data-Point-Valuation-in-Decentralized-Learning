...
for i, _ in enumerate(final_path_list):
    ...
    for seed in seed_list:
        df = pd.read_csv(os.path.join(os.getcwd(), "results", "default"+str(seed)+final_path_list[i], "Shapley_federated_performance.csv"))
        df_list.append(df)

    Path(os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i])).mkdir(parents=True, exist_ok=True)
    
    path_means_and_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_federated_performance_means_and_hs.csv")
    path_means = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_federated_performance_means.csv")
    path_hs = os.path.join(os.getcwd(), "results", "plot_documents"+final_path_list[i], "Shapley_federated_performance_hs.csv")
