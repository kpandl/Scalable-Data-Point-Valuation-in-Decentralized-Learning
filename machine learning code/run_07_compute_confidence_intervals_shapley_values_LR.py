...
final_path_list_gender = ["_reduced_maxtranslation_0.1final_50_50", "_reduced_maxtranslation_0.1final_75_25", "_reduced_maxtranslation_0.1final_100_0", "_reduced_maxtranslation_0.1final_as_is"]
final_path_list_age = ["_reduced_maxtranslation_0.1final_age_quantile_100_0", "_reduced_maxtranslation_0.1final_age_quantile_75_25", "_reduced_maxtranslation_0.1final_age_quantile_50_50", "_reduced_maxtranslation_0.1final_age_as_is"]
...
amounts_of_experiments = [40] * len(final_path_list)
...
for mode in ["Performance", "Bias"]:
    for i, _ in enumerate(final_path_list):
        ...
        for seed in seed_list:
            if(mode == "Performance"):
                df = pd.read_csv(os.path.join(os.getcwd(), "results", "default"+str(seed)+final_path_list[i], "Shapley_LR_method_predictive_performance" + age_addition_pp + "_with_coalition.csv"))
            if(mode == "Bias"):
                df = pd.read_csv(os.path.join(os.getcwd(), "results", "default"+str(seed)+final_path_list[i], "Shapley_LR_method_bias" + age_addition + "_with_coalition.csv"))
            df_list.append(df)
